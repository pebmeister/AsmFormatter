...
bool AsmFormatter::load()
{
    ...
    for (auto& line : lines)
    {
        ...
        if (fmat.commentOnly)
        {
            printAtOffset(outfile, fmat.comment,
                line[0] == ';' ? 0 : m_options.opcodeColumn);
            outfile << "\n";
            continue;
        }
        ...
        if (!fmat.label.empty())
        {
            ...
            if (m_options.requireColon &&
                fmat.label[fmat.label.size() - 1] != ':')
            {
                outfile << ":";
                m_column += 1;
            }
...
void AsmFormatter::printAtOffset(
...
AsmFormatter::AsmLine AsmFormatter::parseLine(const std::string& rawLine) const
{
    ...
    for (size_t i = 0; i < rawLine.length(); ++i)
    {
        ...
        // If we hit a semicolon outside of any quotes,
        // we found the true comment start.
    ...
    if (!codePart.empty() &&
        !std::isspace(static_cast<unsigned char>(codePart[0])))
    {
        while (idx < codePart.length() &&
               !std::isspace(static_cast<unsigned char>(codePart[idx])))
        {
            ++idx;
        }
    ...
    // Advance idx to find the second token
    size_t lookAheadIdx = idx;

    while (lookAheadIdx < codePart.length() &&
           std::isspace(static_cast<unsigned char>(codePart[lookAheadIdx])))
    {
        ++lookAheadIdx;
    }
    ...
    if (lookAheadIdx < codePart.length())
    {
        ...
        while (tokenEnd < codePart.length() &&
               !std::isspace(static_cast<unsigned char>(codePart[tokenEnd])))
        {
            ++tokenEnd;
        }
    ...
    if (idx < codePart.length())
    {
        ...
        if (line.opcode.empty())
        {
            ...
            while (opEnd < remaining.length() &&
                   !std::isspace(static_cast<unsigned char>(remaining[opEnd])))
            {
                ++opEnd;
            }
...
std::vector<std::string> split_by_chars(
    ...
    std::string_view delimiters)
{
    ...
    // Handle trailing delimiter edge case
    if (!sv.empty() &&
        delimiters.find(sv.back()) != std::string_view::npos)
    {
        tokens.emplace_back("");
    }
